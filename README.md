...
**As invariantes são executáveis.** Idempotência, comutatividade, monotonicidade e convergência não são afirmadas — são propriedades verificadas sobre qualquer sequência gerada. São **85 invariantes** na suíte.
...
bundle exec rspec        # 158 exemplos, sem nenhuma dependência externa
```
...
cd harness && bundle exec rspec  # 192 exemplos
bin/sabotage                     # 6 violações deliberadas
bin/mutate                       # 17 mutações
```

| Verificação | Resultado |
|---|---|
| Suíte completa | **192 exemplos, 0 falhas** |
| Invariantes | **85** |
| Mutações mortas | **17 / 17** |
...
| **Pilar 3** — Testes de propriedade | [`spec/properties/`](harness/spec/properties) | 85 invariantes |
